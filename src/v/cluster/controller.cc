...
ss::future<> controller::create_cluster() {
    ...
    return _stm
      .invoke_on(
        ...
        [this, cmd_data = std::move(cmd_data)](controller_stm& stm) mutable {
            model::record_batch b = serde_serialize_cmd(
              bootstrap_cluster_cmd{0, std::move(cmd_data)});
      ...
      .then([](std::error_code errc) {
          vassert(errc == errc::success, "TODO: failed with {}", errc);
      });
}
...
controller::cluster_creation_hook(const bool local_node_is_seed_server) {
    ...
    if (!local_node_is_seed_server) co_return;
